...
ALTER TABLE moct_link ADD COLUMN min_cost real; -- mock_link 테이블 수정 및 min_cost란 열 추가
UPDATE moct_link SET min_cost = (length / (max_spd::real / 36) / 10); -- min_cost / 속도를 변환 km/h -> m/s 변환
CREATE INDEX min_cost_idx ON public.moct_link(min_cost); -- creat index 생성할 인덱스 생성
...
INSERT INTO temp_points (geom) VALUES 
    ...
    (ST_SetSRID(ST_Point(127.1023,  36.8), 4326)), -- 음식점 / 스시슌
    (ST_SetSRID(ST_Point(127.1489231, 36.8098636), 4326)), -- 경유지 / 천안역전시장
    (ST_SetSRID(ST_Point(127.1319896, 36.7790622), 4326)), -- 경유지 / 신방공원
    (ST_SetSRID(ST_Point(127.1297, 36.8273), 4326)), -- 경유지 / 듀팡과자점
    (ST_SetSRID(ST_Point(127.1115, 36.8108), 4326)); -- 숙소 / 천안스위트
...
DROP TABLE IF EXISTS closest_node_result;
...
INNER JOIN (
    ...
    FROM
        ...
        (SELECT node_id::bigint FROM closest_node_result WHERE point_id = 5),
        (SELECT node_id::bigint FROM closest_node_result WHERE point_id = 6)
    ]
        )
) AS d
ON
...
-- 총 거리 계산
select max(agg_cost)/1000 as "cost(km)" from dijkstraVia_result;
